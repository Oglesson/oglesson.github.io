<script lang="ts">
	...
	let {
		/** Icon fill colour (any CSS colour / custom property). */
		color = 'var(--dark)',
		/** 'row', 'column', or 'row-then-column' (row on mobile, column at ≥600px). */
		layout = 'row'
	}: {
		color?: string;
		layout?: 'row' | 'column' | 'row-then-column';
	} = $props();
</script>

<ul class="social-links {layout}" style:--icon-color={color}>
	{#each socialLinks as link (link.id)}
		<li>
			<!-- eslint-disable svelte/no-navigation-without-resolve -- entries in social-links.ts are always external URLs -->
			<a
				href={link.href}
				aria-label={link.ariaLabel}
				target="_blank"
				rel="noopener external me"><span data-icon={link.icon}>{link.label}</span></a
			>
...
<style>
	...
	span {
		...
		width: 2.5rem;
		height: 2.5rem;
		background-color: var(--icon-color, var(--dark));
