...
describe('skills.svelte', () => {
	...
	it('skill chips are collapsed by default', async () => {
		render(Skills);
		const heading = page.getByRole('heading', { name: 'Main Skills' }).element();
		const openCount = heading.ownerDocument.querySelectorAll('details[open]').length;
	...
	it('clicking a skill chip expands it', async () => {
		...
		const svelteText = page.getByText('Svelte', { exact: true }).element();
		const isOpen = svelteText.closest('details')?.hasAttribute('open') ?? false;
